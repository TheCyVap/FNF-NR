...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		var seedInfo:FlxText = new FlxText(20, 15 + 64, 0, "Seed: ", 32);
		seedInfo.text += Main.RCurrentSeed;
		if (Main.RandomizeOn == false)
		{
		seedInfo.text = "";
		}
		seedInfo.scrollFactor.set();
		seedInfo.setFormat(Paths.font("vcr.ttf"), 32);
		seedInfo.updateHitbox();
		add(seedInfo);
		...
		seedInfo.x = FlxG.width - (seedInfo.width + 20);

		FlxTween.tween(bg, {alpha: 0.6}, 0.4, {ease: FlxEase.quartInOut});
		FlxTween.tween(levelInfo, {alpha: 1, y: 20}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.3});
		FlxTween.tween(levelDifficulty, {alpha: 1, y: levelDifficulty.y + 5}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.5});
		FlxTween.tween(seedInfo, {alpha: 1, y: seedInfo.y + 5}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.7});
