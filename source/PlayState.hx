...
class PlayState extends MusicBeatState
{
	...
	var creditTxt2:FlxText;
	var creditTxtScroll:Int = 0;
	var LastNotePos:Int = 0;
	var NoteAlt:Int = 0;
	...
	Main.RightBotH = false;
		...
		{
		creditTxt = new FlxText(5, FlxG.height - 48,0,"", 16);
		}
		else
		{
		creditTxt = new FlxText(5, 26, 0,"", 16);
		}
		creditTxt.scrollFactor.set();
		creditTxt.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		creditTxt2 = new FlxText(5, FlxG.height*2,0,"", 16);
		creditTxt2.scrollFactor.set();
		creditTxt2.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (Main.BotDemo == 1)
		{
			Alter = "[Randomized Demo]";
		}

		// Updating Discord Rich Presence.
		DiscordClient.changePresence(detailsText + " " + SONG.song + " "+Alter+" (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);	
		#end
		...
		//For easy access to making stages work lol
		var validStage = 0;
		switch(SONG.stage)
		{
			case 'halloween': 
		...
		if (storyWeek > 1 && validStage == 0 && SONG.song.toLowerCase() != 'test')
		{
			if (storyWeek == 2)
		...
		if (Main.ROpponents == 1)
		{
			...
			switch (opp)
			{
				...
				case 4:
					...
					if (SONG.stage == 'limo')
					{
					SONG.player2 = "mom-car";
					}
				case 5:
					SONG.player2 = "parents-christmas";
				case 6:
					SONG.player2 = "monster-christmas";
				case 7:
					SONG.player2 = "monster";
				case 8:
					SONG.player2 = "senpai";
				case 9:
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				var songName = new FlxText(songPosBG.x + (songPosBG.width / 2),songPosBG.y-4,0,SONG.song, 16);
				if (FlxG.save.data.downscroll)
					songName.y -= 3;
		...
		//
			...
			if (creditTxt.text != "")
			{
				add(creditTxt);
				add(creditTxt2);
				creditTxt2.y = FlxG.height*2;
				creditTxt.cameras = [camHUD];
				if (creditTxtScroll == 1)
				{
				creditTxt.x = 0;
				}
			}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (Main.ExtremeMode == 1)
		{
			var FunniNote:Note = new Note(songData.bpm*2, 0, null, false, false); 
			FunniNote.sustainLength = 0;
			FunniNote.scrollFactor.set(0, 0);
			unspawnNotes.push(FunniNote);
			FunniNote.visible = false;
		}
		// NEW SHIT
		if (Main.RandomizeOn == true)
		{
		...
		{
		Main.RCurrentSeed = Main.RandomizeSeed;
		FlxG.random.initialSeed = Main.RCurrentSeed;
		}
		}
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (Main.RandomizeOn == true && NoteAlt == 0)
				{
					while (Main.daPos < songNotes[0])
					{
						Main.daPos += 4;
						fuck = FlxG.random.int(0, 3);
					}
				}
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			DiscordClient.changePresence("PAUSED on " + SONG.song + " "+Alter+" (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
			#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			{
				DiscordClient.changePresence(detailsText, SONG.song + " "+Alter+" (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), iconRPC);
			}
			#end
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (creditTxtScroll == 1)
		{
			...
			creditTxt.x -= 6 / (fakeFramerate);
			if (creditTxt.x <= 0-creditTxt2.width)
			{
				creditTxt.x += creditTxt2.width-2;
			}
		}
		...
		if (health <= 0 && Main.BotMode == 0)
		{
			...
			DiscordClient.changePresence("GAME OVER -- " + SONG.song + " "+Alter+" (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy),"\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
			#end
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (SONG.notes[Math.floor(curStep / 16)] != null)
						{
							if (SONG.notes[Math.floor(curStep / 16)].altAnim)
							{
								...
								if (Main.ROpponents == 1 && SONG.player2 != 'parents-christmas')
								{
								altAnim = '';
								}
							}
						...
						dad.holdTimer = 0;
						Main.EDance = Math.round((openfl.Lib.current.stage.frameRate)/2);
					...
					{
						if ((daNote.isSustainNote && daNote.wasGoodHit) || Main.BotMode > 0)
						{
							daNote.kill();
							notes.remove(daNote, true);
							daNote.destroy();
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				if (Main.BotMode == 1)
				{
				holdArray = [Main.LeftBotH, Main.DownBotH, Main.UpBotH, Main.RightBotH];
				pressArray = [
					Main.LeftBot,
					Main.DownBot,
					Main.UpBot,
				...
				if (pressArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
				{
					...
					{
						if (pressArray[i] && !directionList.contains(i) && Main.BotMode == 0)
							dontCheck = true;
					}
	...
	*/
	...
	override function stepHit()
	{
		...
		// Updating Discord Rich Presence (with Time Left)
		DiscordClient.changePresence(detailsText + " " + SONG.song + " "+Alter+" (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC,true,  songLength - Conductor.songPosition);
		#end

		if (Main.BotDemo == 1)
		{
		DiscordClient.changePresence(detailsText + " " + SONG.song + " "+Alter+"", iconRPC,true,  songLength - Conductor.songPosition);
		}
	}
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'sc-milf' && curBeat >= 168 && curBeat < 200)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
